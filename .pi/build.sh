...
# docker buildx build --tag cosmostriliza --file .pi/Dockerfile --platform linux/arm64 --cache-from cosmostriliza:cache --cache-to cosmostriliza:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar cosmostriliza
...
sudo bash -c "echo cosmostriliza > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/cosmostriliza.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/cosmostriliza.img
